...
// ====== Booking Handler ======
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['appointmentDate'], $_POST['timeSlot'], $_POST['serviceID'])) {
    ...
    if (!$appointmentDate || !$timeSlot || !$serviceID) {
        echo "<script>alert('Missing required fields.'); window.location.href='gold_services.php';</script>";
        exit();
    }

    // Max 3 per service per date
    ...
    // Insert appointment
    $stmt = $conn->prepare("INSERT INTO appointments (userID, serviceID, appointmentDate, timeSlot, status) VALUES (?, ?, ?, ?, ?)");
